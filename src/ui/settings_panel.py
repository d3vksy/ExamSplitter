...
class SettingsPanel(ttk.LabelFrame):
    def __init__(self, parent: Any, callback: Optional[Callable] = None, detector: Optional[Any] = None) -> None:
    ...
    def setup_ui(self) -> None:
        ...
        self.confidence_label = ttk.Label(
            basic_frame, text=f"현재: {self.confidence_var.get():.1f}"
        )
        ...
        seed_spinbox = ttk.Spinbox(
            seed_frame,
            from_=1,
            to=9999,
            textvariable=self.shuffle_seed_var,
            command=self.on_setting_changed,
            width=10,
        )
        ...
        ttk.Label(
            shuffle_frame,
            text="※ 고정 시드를 사용하면 항상 같은 순서로 셔플됩니다",
            font=("", 8),
            foreground="gray",
    ...
    def on_setting_changed(self, *args: Any) -> None:
    ...
    def on_model_changed(self, event: Optional[Any] = None) -> None:
        """모델이 변경되었을 때 호출됩니다."""
        # 콜백 즉시 호출
        if self.callback:
            self.callback()
    ...
    def update_available_models(self) -> None:
        ...
        try:
            ...
            # 기본값 설정
            if (
                not self.selected_model_var.get()
                or self.selected_model_var.get() not in model_names
            ):
        ...
        try:
            ...
            if not model_path.exists():
                self.model_info_label.config(
                    text=f"모델 파일을 찾을 수 없음: {selected_model}"
                )
                return
            ...
            if self.detector:
                ...
                if (
                    current_model_info["loaded"]
                    and current_model_info["name"] == selected_model
                ):
                    status = "로드됨 ✅"
                else:
                    status = "로드 안됨 ⚠️"
    ...
    def get_settings(self) -> Dict[str, Any]:
        return {
            ...
            "output_formats": {
                name: var.get() for name, var in self.output_formats.items()
            },
            "shuffle_seed": (
                self.shuffle_seed_var.get() if self.use_random_seed_var.get() else None
            ),
            "selected_model": self.selected_model_var.get(),
        }
